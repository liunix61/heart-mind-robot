...
void LAppWavFileHandler::Start(const Csm::csmString &filePath) {
    ...
    _lastRms = 0.0f;
    // 读取音频文件数据
    NSData *audioData = [NSData dataWithContentsOfFile:[NSString stringWithUTF8String:filePath.GetRawString()]];
    if (audioData) {
        _audioPlayer->playAudio((const char*)audioData.bytes, audioData.length);
    }
}
